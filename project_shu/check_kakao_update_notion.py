...
def newPage(title, name, start, end) :
    page_values = {
        'title' : title,
        'name' : name,
...
def create_page(information, note) :
    name = information[0]
    tool_list = information[1]
    date_start = [2023] + information[2][0]
    date_end = [2023] + information[2][1]
    purpose = information[3]

    if('녹음실' in tool_list):
        page_id = newPage('녹음실 대여', name, date_start, date_end)
        objs = []
    ...
    if not ('녹음실' in tool_list and len(tool_list) == 1) :
        page_id = newPage('장비 내부 대여', name, date_start, date_end)
        objs = []
        objs.append(title('대여 장비'))
        for tool in c.LIST[8:]:
            objs.append(check(tool, tool in tool_list))
        objs.append(text(''))
        objs.append(title('비고 (SD카드, 삼각대는 개수 작성 필수!)'))
...
def checkDate(dates):
    ...
    if(dates[1][2] >= 24):
        dates[1][2] -= 24
        exceed[1] = True
    start = datetime.datetime(2023, *dates[0])
    end = datetime.datetime(2023, *dates[1])
    oneday = datetime.datetime(2023,1,2) - datetime.datetime(2023,1,1)
    ...
    for i in range(len(replaced)):
        ...
        try:
            if(tool_able or not fatal):
                create_page(replaced[i], create_note(fitted[i][1]))
        except:
